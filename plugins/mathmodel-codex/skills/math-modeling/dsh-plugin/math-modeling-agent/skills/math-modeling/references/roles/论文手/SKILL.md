...
摘要、关键词、参考文献和官方要求的附录仍须完整提供。若题目类型、论证逻辑或官方模板适合更好的章节框架，先向用户提出具体结构建议和理由；获得用户选择前，按上述默认框架推进可逆的准备工作，不擅自改变最终论文结构。

正文以约 15000 个中文字符、约 20 页为常规交付目标，并尽量不低于该目标。该目标指有实质论证内容的正文，不得通过重复叙述、空泛扩写、异常字号、行距或大图堆叠凑字数和页数。若官方页数上限、用户明确要求或题目实际内容与目标冲突，以官方规则和内容质量为先，并在写作开始前向用户说明预计篇幅；交付时报告实际正文字符数和实际渲染页数。15000 字和 20 页是项目质量目标，不得虚构为竞赛官方最低要求。
