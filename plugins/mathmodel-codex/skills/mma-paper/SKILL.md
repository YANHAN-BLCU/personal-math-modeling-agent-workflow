---
name: mma-paper
description: 使用内置 CUMCM LaTeX 模板完成数学建模竞赛的建模、逐问求解、论文撰写和高级绘图。当用户要完成数学建模题、撰写或完善数学建模论文、生成 CUMCM 论文，或处理含 document.tex 与 cumcmthesis.cls 的项目时使用。
---
...
先定位本 Skill 所在目录（即包含本 `SKILL.md` 的 `mma-paper` 目录），将其中的
`assets/template/` 完整复制到当前工作目录。不要依赖固定的用户主目录路径。
